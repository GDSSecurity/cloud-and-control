...
SELECT 
	...
	r.logfile,
	SUBSTRING_INDEX(
		left(REPLACE(r.wordlist, SUBSTRING_INDEX(r.wordlist, '_', -2), ''),
			LENGTH(REPLACE(r.wordlist, SUBSTRING_INDEX(r.wordlist, '_', -3), '')) - 1),
		'_', -1)
                as wordlist
FROM (
...
echo "\n";
$sql = "INSERT INTO result_hashcat7zparsed ";
for($i=0; $i < count($results); $i++)
  {
    $sql .= ($i != 0) ? " UNION ALL\nSELECT " : "\nSELECT ";
...
echo $sql . "\n\n";
$result = mysql_query("DROP TABLE IF EXISTS result_hashcat7zparsed", $conn) or die( mysql_error($conn)."\n");
$result = mysql_query("CREATE TABLE result_hashcat7zparsed (id INT, linesinfile INT, cracked INT, cryptsec INT, total_sec INT, wordlist varchar(400))", $conn) or die( mysql_error($conn)."\n");
