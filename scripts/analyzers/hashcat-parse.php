...
CONCAT(TRIM(TRAILING '0' FROM r.name), '2') as logfile,
...
echo "\n";
$sql = "INSERT INTO result_hashcatparsed ";
for($i=0; $i < count($results); $i++)
  {
    $sql .= ($i != 0) ? " UNION ALL\nSELECT " : "\nSELECT ";
...
//echo $sql . "\n\n";
$result = mysql_query("DROP TABLE IF EXISTS result_hashcatparsed", $conn) or die( mysql_error($conn)."\n");
$result = mysql_query("CREATE TABLE result_hashcatparsed (id INT, linesinfile INT, cracked INT, cryptsec INT, total_sec INT, wordlist varchar(400))", $conn) or die( mysql_error($conn)."\n");
